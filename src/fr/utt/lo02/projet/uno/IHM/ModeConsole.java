package fr.utt.lo02.projet.uno.IHM;
...
{	
	...
	public int askCarte(Joueur joueur) //Permet au joueur de choisir une carte
	{
		...
		for(int i = 0; i<partie.getNbreJoueur(); i++)
		{
			System.out.println(partie.getJoueur(i).afficherPseudo()+ ": "+partie.getJoueur(i).getNombreCarte()+" cartes restantes.");
		}
	...
	/** 
	 ...
	 {
		  if(partie.getJoueur(partie.getJoueurActuel()) == joueur && joueur instanceof JoueurNormal)
				 	System.out.println("Vous avez pioché un " + joueur.getMain().getMain().get(joueur.getNombreCarte()-1));
			 else 
				 	System.out.println(joueur.afficherPseudo() + " a pioché.");
